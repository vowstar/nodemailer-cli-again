...
var cli = function(options) {
  this.options = {
    ...
    'string': ['server', 'service', 'username', 'password', 'subject', 'attachment', 'proxy']
  };
...
cli.prototype.parse = function(argv, next) {
  ...
  if (options.version) {
    ...
    if (options.proxy) {
      if (!(options.proxy.includes('http://') ||
            options.proxy.includes('https://') ||
            options.proxy.includes('socks5://') ||
            options.proxy.includes('socks://') ||
            options.proxy.includes('socks4://') ||
            options.proxy.includes('socks4a://'))) {
        this.errors.push(new Error("A proxy URL must start with http://, https://, socks5://, socks4:// or socks://."));
      }
    }
